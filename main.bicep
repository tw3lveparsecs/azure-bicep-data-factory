...
param diagnosticSettingsName string = '${dataFactoryName}-diagnosticSettings'
...
resource managedVirtualNetwork 'Microsoft.DataFactory/factories/managedVirtualNetworks@2018-06-01' = if (!empty(managedVirtualNetworkName)) {
...
resource managedIntegrationRuntime 'Microsoft.DataFactory/factories/integrationRuntimes@2018-06-01' = if (enableManagedVnetIntegrationRuntime) {
  name: '${uniqueString(deployment().name, location)}-DataFactory-IntegrationRuntimes'
  parent: dataFactory
  properties: {
    type: 'Managed'
